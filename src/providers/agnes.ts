...
export interface VideoProvider {
  create(input: VideoInput): Promise<VideoStatus>;
...
export class AgnesProvider implements VideoProvider, TextProvider {
  ...
  private async request(url: string, body?: unknown) {
    ...
    if (!response.ok)
      throw new ProviderError(response.status, !!body && response.status >= 500);
    try {
      ...
      messages: [
        {
          role: "system",
          content:
            "Return only the requested JSON. Do not invent facts, statistics or quotations. Clearly distinguish speculation and hypothetical scenarios from facts. Never add captions inside visual frames.",
        },
  ...
  async create(input: VideoInput) {
    ...
    if (input.mode === "keyframe") {
      if (!input.firstFrame && !input.lastFrame)
        throw new Error("A keyframe image is required");
      body.first_frame = input.firstFrame;
      body.last_frame = input.lastFrame;
    }
    if (input.mode === "reference") {
      if (!input.images?.length && !input.audios?.length && !input.videos?.length)
        throw new Error("Reference media is required");
      body.images = input.images;
      body.audios = input.audios;
      body.videos = input.videos;
    }
    const raw = await this.request(`${this.base()}/videos`, body);
    try {
      return this.map(raw);
    } catch {
  ...
  async getStatus(id: string) {
    const url = new URL("/agnesapi", this.base());
  ...
  private map(raw: unknown): VideoStatus {
    const r = responseSchema.parse(raw);
    return { id: r.video_id, status: r.status, url: r.metadata?.url };
  }
}
export interface NarrationProvider {
  generate(text: string): Promise<{ path: string; duration: number }>;
}
// No standalone TTS endpoint is documented in the official index. Uploads are the MVP boundary.
export class UploadedNarrationProvider implements NarrationProvider {
