...
export async function saveUpload(
  ...
  if (file.size > MAX_UPLOAD || file.size < 1)
  ...
  if (
    !type ||
    !allowed.includes(type.mime) ||
    (file.type &&
      file.type !== type.mime &&
      !(file.type === "audio/wav" && type.mime === "audio/x-wav"))
  )
    throw new Error("Unsupported or mismatched media type");
  ...
  try {
    ...
    if (kind === "reference") {
      ...
      if (
        ...
        !video.width ||
        !video.height ||
        video.width * video.height > 40000000
      )
      ...
      throw new Error("Audio must be between 4 and 600 seconds");
