...
export async function generateShot(
  ...
  const shot = await db.query.shots.findFirst({
    where: and(eq(shots.id, job.shotId!), eq(shots.generationId, g.id)),
  });
  if (!shot || shot.version !== job.version || shot.status === "COMPLETED") return true;
  if (job.providerId) {
    if (
      job.polls >= 180 ||
      Date.now() - new Date(job.startedAt || job.createdAt).getTime() > 2 * 3600000
    )
      throw new Error("Video polling deadline reached");
    ...
    if (status.status !== "completed") {
      await db
        ...
        .set({
          status: "SUBMITTED",
          polls: job.polls + 1,
          runAfter: new Date(Date.now() + Math.min(30000, 3000 + job.polls * 500)),
        })
    ...
    await db.transaction(async (tx) => {
      ...
      const [clip] = await tx
        ...
        .values({
          ...
          metadata: { shotId: shot.id, version: job.version },
        })
        .returning();
      ...
      await tx
        ...
        .where(and(eq(shots.id, shot.id), eq(shots.version, job.version)));
    });
    await refreshGeneration(g.id);
    return true;
  }
  if (shot.status === "SUBMITTING" || shot.status === "UNCERTAIN") {
    await uncertain(job, shot.id);
    return true;
  }
  const input: VideoInput = {
    prompt: `${shot.videoPrompt}\nCamera: ${shot.camera}. Environment: ${shot.environment}. Visual bible: ${JSON.stringify(g.visualBible)}. No text overlays or captions.`,
    seconds: shot.duration,
    aspectRatio: g.aspectRatio,
    mode: shot.mode as VideoInput["mode"],
  };
  let reference = shot.referenceId;
  if (shot.characterId) {
    const character = await db.query.characters.findFirst({
      where: and(eq(characters.id, shot.characterId), eq(characters.userId, g.userId)),
    });
    if (character) {
      input.prompt += `\nCharacter: ${character.description}. ${character.visualPrompt}`;
      reference ??= character.referenceId;
    }
  }
  if (shot.continuityFrom) {
    const previous = await db.query.shots.findFirst({
      where: and(eq(shots.id, shot.continuityFrom), eq(shots.generationId, g.id)),
    });
    if (!previous?.clipId) {
      await db
        .update(jobs)
        .set({ status: "QUEUED", runAfter: new Date(Date.now() + 15000) })
        .where(eq(jobs.id, job.id));
      return false;
    }
    const clip = await db.query.assets.findFirst({
      where: and(eq(assets.id, previous.clipId), eq(assets.userId, g.userId)),
    });
  ...
  const [claimed] = await db
    ...
    .where(
      and(
        eq(shots.id, shot.id),
        eq(shots.version, job.version),
        eq(shots.status, "QUEUED"),
      ),
    )
    .returning();
  if (!claimed) return true;
  try {
    const status = await provider.create(input);
    await db.transaction(async (tx) => {
      await tx
        .update(jobs)
        .set({
    ...
    // The clip is not ours yet; polling continues through the database outbox.
    return false;
  } catch (error) {
    ...
    await db.update(shots).set({ status: "QUEUED" }).where(eq(shots.id, shot.id));
    throw error;
  }
}
export async function uncertain(job: Job, shotId: string) {
  await db
    .update(jobs)
    .set({
      status: "UNCERTAIN",
      error:
        "Submission outcome unknown; verify provider account before explicitly retrying.",
    })
    .where(eq(jobs.id, job.id));
  await db
    .update(shots)
    .set({
      status: "UNCERTAIN",
      lastError: "Submission outcome unknown. A manual retry may incur another charge.",
    })
    .where(and(eq(shots.id, shotId), eq(shots.version, job.version)));
  await refreshGeneration(job.generationId);
}
