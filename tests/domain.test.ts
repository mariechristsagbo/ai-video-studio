...
describe("narration-first planning", () => {
  it("balances short shots without a tiny remainder", () => {
    ...
    for (const total of [30, 60, 90, 180, 300, 182.4]) {
      const shots = planShotDurations(total);
      expect(shots.reduce((a, b) => a + b, 0)).toBe(Math.ceil(total));
  ...
  it("never resubmits ambiguous or already submitted work", () => {
    for (const state of ["SUBMITTING", "UNCERTAIN", "SUBMITTED", "COMPLETED"])
      expect(shouldSubmit(state)).toBe(false);
    expect(shouldSubmit("QUEUED")).toBe(true);
  });
  it("builds a narration-aligned composition", () => {
    const c = buildComposition(
      [
        { path: "a", duration: 7, transition: "cut" },
        { path: "b", duration: 7, transition: "cut" },
      ],
      "9:16",
      13.4,
    );
    expect(c.duration).toBe(13.4);
    expect(c.width).toBe(1080);
    expect(c.video[1].start).toBe(7);
  });
});
